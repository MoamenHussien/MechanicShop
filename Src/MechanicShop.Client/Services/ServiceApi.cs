...
public class ServiceApi(IHttpClientFactory httpClientFactory, TimeZoneService timeZoneService)
{
    ...
    // Vehicle methods
    public async Task<ApiResult<List<VehicleResponse>>> GetVehiclesByCustomerIdAsync(Guid customerId)
    {
        try
        {
            ...
            {
                return ApiResult<List<VehicleResponse>>.Failure(
            ...
            return ApiResult<List<VehicleResponse>>.Success(vehicles);
        }
        catch (Exception ex)
        {
            return await HandleExceptionAsync<List<VehicleResponse>>(ex, $"Failed to retrieve vehicles for customer {customerId}");
        }
    }
    ...
    public async Task<ApiResult> UpdateCustomerAsync(Guid customerId, UpdateCustomerRequest request)
    {
        try
        {
        ...
        {
            return await HandleExceptionAsync(ex, $"Failed to update customer {customerId}");
        }
    }
    ...
    public async Task<ApiResult<List<VehicleMakeModel>>> GetVehicleMakesAsync()
    {
        try
        {
            var response = await _httpClient.GetAsync("api/v1/makes");
    ...
    public async Task<ApiResult<List<VehicleModelItemModel>>> GetVehicleModelsByMakeIdAsync(Guid makeId)
    {
        try
        {
            var response = await _httpClient.GetAsync($"api/v1/makes/{makeId}");
