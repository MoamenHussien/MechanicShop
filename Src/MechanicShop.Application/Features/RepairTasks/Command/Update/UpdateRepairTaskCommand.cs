...
public sealed record UpdateRepairTaskCommand(Guid id,string name, decimal LaborCost, RepairDurationInMinute duration, List<UpdatePartCommand> Parts) :IRequest<Result<Updated>>;
...
public sealed class UpdateRepairTaskCommandValidator : AbstractValidator<UpdateRepairTaskCommand>
{
    public UpdateRepairTaskCommandValidator()
    {
        ...
        RuleFor(n=>n.Parts).NotNull().WithMessage("Repair Task Parts Is Required").Must(n=>n.Count>0).WithMessage("At least one part is required");
